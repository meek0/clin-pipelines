...
case class TTask(submissionSchema: Option[String], taskExtensions: TaskExtensions) {
  ...
  def buildResource(analysisRef: Option[Reference], sequencingRef: Reference, patient: Reference, requester: Reference, sample: Reference, drr: DocumentReferencesResources, batchId: String): Resource = {
    ...
    val variantCallOutput = {
      ...
      code.addCoding()
        .setSystem(CodingSystems.DR_TYPE)
        .setCode(if (EXTUM_SCHEMA.equals(submissionSchema.orNull)) VariantCalling.documentTypeSomatic else VariantCalling.documentTypeGermline)
    ...
    val cnvOutput = {
      ...
      code.addCoding()
        .setSystem(CodingSystems.DR_TYPE)
        .setCode(if (EXTUM_SCHEMA.equals(submissionSchema.orNull)) CopyNumberVariant.documentTypeSomatic else CopyNumberVariant.documentTypeGermline)
    ...
    val structuralVariantOutput = if (drr.structuralVariant != null) {
      ...
      code.addCoding()
        .setSystem(CodingSystems.DR_TYPE)
        .setCode(if (EXTUM_SCHEMA.equals(submissionSchema.orNull)) StructuralVariant.documentTypeSomatic else StructuralVariant.documentTypeGermline)
      val sup = new TaskOutputComponent()
        .setType(code)
