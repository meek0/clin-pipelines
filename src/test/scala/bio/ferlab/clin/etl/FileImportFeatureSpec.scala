...
class FileImportFeatureSpec extends FlatSpec with WholeStackSuite with Matchers {
  ...
  "run" should "return no errors" in {
    withS3Objects { (inputPrefix, outputPrefix) =>
      ...
      //Expected title
      documentReferences.flatMap(d => d.getContent.asScala.map(_.getAttachment.getTitle)) should contain only(
        "file1.cram", "file1.cram.crai", "file2.vcf.gz", "file2.vcf.gz.tbi", "file4.vcf.gz", "file4.vcf.gz.tbi",
        "file5.vcf.gz", "file5.vcf.gz.tbi", "file3.json", "file6.html","file6.json","file6.variants.tsv","file7.seg.bw",
        "file7.baf.bw","file7.roh.bed","file7.exome.bed","file8.png","file9.csv","file10.json")
